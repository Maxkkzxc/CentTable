...
function Dashboard() {
  ...
  const [createColumns, setCreateColumns] = useState([
    { name: '', type: 'String', validationRegex: '', options: '', externalTable: '', externalColumn: '' }
  ]);
  ...
  const resetCreateForm = () => {
    ...
    setCreateColumns([{ name: '', type: 'String', validationRegex: '', options: '', externalTable: '', externalColumn: '' }]);
  };

  const handleAddCreateColumn = () => {
    setCreateColumns([...createColumns, { name: '', type: 'String', validationRegex: '', options: '', externalTable: '', externalColumn: '' }]);
  };
  ...
  const handleCreateColumnChange = (index, field, value) => {
    ...
    if (field === 'type' && value !== 'External') {
      newCols[index].externalTable = '';
      newCols[index].externalColumn = '';
    }
  ...
  const handleOpenEdit = (grid) => {
    ...
    const cols = grid.columns.map(col => ({
      ...
      externalColumn: col.externalColumn || ''
    }));
    setEditColumns(cols);
    setOpenEdit(true);
  };
  ...
  const handleAddEditColumn = () => {
    setEditColumns([...editColumns, { id: 0, name: '', type: 'String', validationRegex: '', options: '', externalTable: '', externalColumn: '' }]);
  };
  ...
  const handleEditGrid = async () => {
    const processedColumns = editColumns
      ...
      .map(({ id, name, type, validationRegex, options, externalTable, externalColumn }) => ({
        id, name, type, validationRegex, options, externalTable, externalColumn
      }));
          ...
          {createColumns.map((col, index) => (
            <Box key={index} sx={{ mb: 1, display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
              ...
              {col.type === 'External' && (
                <>
                  <TextField
                    label="Внешняя таблица"
                    value={col.externalTable}
                    onChange={(e) => handleCreateColumnChange(index, 'externalTable', e.target.value)}
                    variant="outlined"
                    size="small"
                    sx={{
                      minWidth: 150,
                  ...
                  />
                  <TextField
                    label="Внешний столбец"
                    value={col.externalColumn}
                    onChange={(e) => handleCreateColumnChange(index, 'externalColumn', e.target.value)}
                    variant="outlined"
                    size="small"
                    sx={{
                      minWidth: 150,
              ...
              {col.type === 'External' && (
                <>
                  <TextField
                    label="Внешняя таблица"
                    value={col.externalTable}
                    onChange={(e) => handleEditColumnChange(index, 'externalTable', e.target.value)}
                    variant="outlined"
                    size="small"
                    sx={{
                      minWidth: 150,
                  ...
                  <TextField
                    label="Внешний столбец"
                    value={col.externalColumn}
                    onChange={(e) => handleEditColumnChange(index, 'externalColumn', e.target.value)}
                    variant="outlined"
                    size="small"
                    sx={{
                      minWidth: 150,
